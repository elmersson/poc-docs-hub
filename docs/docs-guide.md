...
Aggregation (`scripts/aggregate.py`) copies each repo's `docs/` into `teams/<team>/<service>/` (grouped by the owning team from `catalog-info.yaml`). The hub documents itself too: its own pages are read from `platform-docs/`, not `docs/`, because `docs/` here is the generated site output — aggregation prefers `platform-docs/` over `docs/` whenever a repo has it. It injects an ownership/freshness banner below the top heading of every aggregated page: the page's `owner`, its `last_reviewed` date, and an **edit at source** link back to the page in its home repo. The link is built from `--github-owner`; without that flag the banner still shows owner and review date, but omits the link. Edit those pages at the source, never here. Aggregation also writes a `.pages` file into each service folder so its sidebar follows a logical order (getting-started, how-to, guides, reference, testing, runbooks, adr) rather than alphabetical; the awesome-pages plugin reads it at build time.
